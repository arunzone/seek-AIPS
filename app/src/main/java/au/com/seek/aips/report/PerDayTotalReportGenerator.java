...
class PerDayTotalReportGenerator {
  ...
  List<TotalVehicleByDate> totalByDate(List<VehicleTraffic> vehicles) {
    Map<LocalDate, Long> totalReportMap = vehicles.stream().
        map(vehicle -> new TotalVehicleByDate(vehicle.getTimestamp().toLocalDate(), vehicle.getCount())).
        collect(groupingBy(vehicle -> vehicle.getDate(), summingLong(vehicle -> vehicle.getCount())));
